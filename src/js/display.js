...
const displayController = (() => {
  ...
  const displayInput = section => {
    ...
    container.insertBefore(elementFactory.createInput(inputSection), container.childNodes[0])
  };
  ...
  return { displayItems, displayInput, cancelAddNew, addNewCategory, addItem, editable, saveEditCategory, saveEditItem, deleteCategory, deleteItem }
})();
...
const eventListeners = () => {
  ...
  row.addEventListener("click", event => {
    ...
    if (clicked.className == "addNew") {
    ...
    else if (clicked.tagName == "INPUT" && clicked.parentElement.className == "category") {
      toggleFactory.toggleSelected(clicked)
      displayController.displayItems(parseInt(clicked.parentElement.id.match(/\d+/)))
    } 
      ...
      displayController.addItem()
        ...
        displayController.editable(parseInt(clicked.id.match(/\d+/)), clicked.parentElement.className)
    }
    else if (clicked.className == "save") {
      if (clicked.parentElement.className == "category") {
        displayController.saveEditCategory(parseInt(clicked.id.match(/\d+/)), "category")
      } else {
        displayController.saveEditItem(parseInt(clicked.id.match(/\d+/)), "item")
      }
    }
